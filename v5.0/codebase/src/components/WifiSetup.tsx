...
export const WifiSetup: React.FC<WifiSetupProps> = ({ state, theme }) => {
  ...
  const overlayClass = theme === 'child' ? "bg-white/95" : "bg-black/90";
  const backBtnClass = theme === 'child' 
  ...
  const qrBoxClass = theme === 'child' ? "bg-white shadow-[0_0_20px_rgba(255,127,80,0.15)] border border-orange-100" : theme === 'dev' ? "bg-white/90 shadow-[0_0_20px_rgba(34,197,94,0.2)]" : "bg-white/90 shadow-[0_0_20px_rgba(34,211,238,0.2)]";
  const textTitleClass = theme === 'child' ? "text-orange-500" : theme === 'dev' ? "text-green-400" : "text-cyan-400";
  const textSubClass = theme === 'child' ? "text-orange-400/70" : theme === 'dev' ? "text-green-600" : "text-cyan-600";
  const btnClass = theme === 'child'
  ...
  const spinnerClass = theme === 'child' ? "text-orange-500 drop-shadow-[0_0_12px_rgba(255,127,80,0.5)]" : theme === 'dev' ? "text-green-400 drop-shadow-[0_0_12px_rgba(34,197,94,0.8)]" : "text-cyan-400 drop-shadow-[0_0_12px_rgba(34,211,238,0.8)]";
  const checkClass = theme === 'child' ? "text-orange-500 drop-shadow-[0_0_15px_rgba(255,127,80,0.6)]" : theme === 'dev' ? "text-green-400 drop-shadow-[0_0_15px_rgba(34,197,94,0.8)]" : "text-cyan-400 drop-shadow-[0_0_15px_rgba(34,211,238,0.8)]";
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-4">
            ...
            <div className="flex gap-4 mt-4">
              <button 
                ...
                className={`px-6 py-2 rounded-full text-xs font-medium border transition-all tracking-wider ${theme === 'child' ? 'bg-white/50 text-orange-500 border-orange-200 hover:bg-orange-50' : theme === 'dev' ? 'bg-black/50 text-green-600 border-green-900/50 hover:bg-green-950/50' : 'bg-black/50 text-cyan-600 border-cyan-900/50 hover:bg-cyan-950/50'}`}
              >
                返回
              </button>
              <button 
