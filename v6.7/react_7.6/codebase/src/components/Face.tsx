...
export type FaceState = 'idle' | 'happy' | 'talking' | 'menu' | 'dizzy' | 'crying' | 'naughty' | 'wink' | 'breath' | 'sleep_wake' | 'look_around' | 'look_left' | 'look_right' | 'yawn' | 'lost' | 'celebrate' | 'excited' | 'angry' | 'curious' | 'thinking' | 'surprised' | 'deep_sleep' | 'light_rest' | 'alert';
...
export const Face: React.FC<FaceProps> = ({ state, theme }) => {
  ...
  const leftEyeVariants = {
    ...
    look_around: { height: blink ? 4 : 40 * scale, width: 32 * scale, borderRadius: '16px', x: [0, -20 * scale, -20 * scale, 20 * scale, 20 * scale, 0], y: 0, rotate: 0, scale: 1, opacity: 1 },
    look_left: { height: blink ? 4 : 40 * scale, width: 32 * scale, borderRadius: '16px', x: -20 * scale, y: 0, rotate: 0, scale: 1, opacity: 1 },
    look_right: { height: blink ? 4 : 40 * scale, width: 32 * scale, borderRadius: '16px', x: 20 * scale, y: 0, rotate: 0, scale: 1, opacity: 1 },
  ...
  const mouthVariants = {
    ...
    look_around: { width: 16 * scale, height: 4 * scale, borderRadius: '4px', x: [0, -10 * scale, -10 * scale, 10 * scale, 10 * scale, 0], y: 0, rotate: 0, scale: 1, opacity: 1 },
    look_left: { width: 16 * scale, height: 4 * scale, borderRadius: '4px', x: -10 * scale, y: 0, rotate: 0, scale: 1, opacity: 1 },
    look_right: { width: 16 * scale, height: 4 * scale, borderRadius: '4px', x: 10 * scale, y: 0, rotate: 0, scale: 1, opacity: 1 },
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center relative w-full">
      <div className="flex gap-14 mb-8">
        <motion.div
          style={{ backgroundColor: t.accent_hi, boxShadow: `0 0 20px ${t.accent_hi}B3` }}
      ...
      {state === 'crying' && (
        <div className="absolute top-1/2 left-0 w-full flex justify-center gap-20 pointer-events-none">
          <motion.div 
            ...
            style={{ backgroundColor: `${t.accent_hi}CC`, boxShadow: `0 0 12px ${t.accent_hi}99` }}
