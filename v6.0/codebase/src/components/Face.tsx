...
export type FaceState = 'idle' | 'happy' | 'talking' | 'menu' | 'dizzy' | 'crying' | 'naughty' | 'wink' | 'breath' | 'sleep_wake' | 'look_around' | 'yawn' | 'lost' | 'celebrate' | 'excited' | 'angry' | 'curious' | 'thinking' | 'surprised' | 'deep_sleep' | 'light_rest' | 'alert';
...
export const Face: React.FC<FaceProps> = ({ state, theme }) => {
  ...
  const leftEyeVariants = {
    ...
    look_around: { height: blink ? 4 : 40 * scale, width: 32 * scale, borderRadius: '16px', x: [0, -20, -20, 20, 20, 0], y: 0, rotate: 0, scale: 1, opacity: 1 },
  ...
  const mouthVariants = {
    ...
    look_around: { width: 16 * scale, height: 4 * scale, borderRadius: '4px', x: [0, -10, -10, 10, 10, 0], y: 0, rotate: 0, scale: 1, opacity: 1 },
